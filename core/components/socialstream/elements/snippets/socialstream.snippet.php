 ...
 * SocialStream
 *
 * Snippet to show your social stream data.
 *
 ...
 * For example: socialstream.unread_posts [default=socialstream.unread_posts]
 *
 * &unreadCountKey - (Opt) The key of the unread count cookie data holder.
 * For example: socialstream_unread_data [default=socialstream_unread_data]
 *
 *
 * TEMPlATES:
 *
 * &twitterTpl - (Opt) The chunk that is used for a twitter post.
 * For example: yourTwitter [default=socialstreamTwitter]
 *
 * &facebookTpl - (Opt) The chunk that is used for a facebook post.
 * For example: yourFacebook [default=socialstreamFacebook]
 *
 * &instagramTpl - (Opt) The chunk that is used for a instagram post.
 * For example: yourInstagram [default=socialstreamInstagram]
 *
 * &youtubeTpl - (Opt) The chunk that is used for a youtube post.
 * For example: yourYoutube [default=socialstreamYoutube]
 *
 * &outerTpl - (Opt) The wrapper chunk use [[+output]] within.
 * For example: yourOuter [default=socialstreamOuter]
 *
 *
 * OTHER:
 *
 ...
 * For example: homeSocialstreamPosts [default=socialstreamPosts]
 *
 * &toPlaceholder - (Opt) If u want the output in placeholder instead of a return.
 * For example: socialstream.output [default=NULL]
 *
 */
...
$socialstream = $modx->getService(
    ...
    $modx->getOption(
        ...
        null,
        $modx->getOption('core_path') . 'components/socialstream/'
    ) . 'model/socialstream/',
    []
);
...
$unreadCountPlaceholder = $modx->getOption('unreadCountPlaceholder', $scriptProperties, 'socialstream.unread_posts');
$unreadCountKey         = $modx->getOption('unreadCountKey', $scriptProperties, 'socialstream_unread_data');

$twitterTpl   = $modx->getOption('twitterTpl', $scriptProperties, 'socialstreamTwitter');
$facebookTpl  = $modx->getOption('facebookTpl', $scriptProperties, 'socialstreamFacebook');
$instagramTpl = $modx->getOption('instagramTpl', $scriptProperties, 'socialstreamInstagram');
$youtubeTpl   = $modx->getOption('youtubeTpl', $scriptProperties, 'socialstreamYoutube');
$outerTpl     = $modx->getOption('outerTpl', $scriptProperties, 'socialstreamOuter');

$cache         = $modx->getOption('cache', $scriptProperties, true);
$cacheTime     = (int) $modx->getOption('cacheTime', $scriptProperties, 120);
$cacheKey      = $modx->getOption('cacheKey', $scriptProperties, 'socialstreamPosts');
...
if (!$cache || ($cache && $modx->cacheManager->get($cacheKey) == '')) {
    ...
    $query = $modx->newQuery('SocialStreamItem');
    $query->where($where);
    $query->limit($limit);
    $query->sortby($sortBy, $sortDir);
    $posts = $modx->getCollection('SocialStreamItem', $query);
